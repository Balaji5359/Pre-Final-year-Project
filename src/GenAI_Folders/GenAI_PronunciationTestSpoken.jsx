...
function GenAI_PronunciationTestSpoken() {
    ...
    const [sessionId] = useState('pronunciation-session-' + Math.random().toString(36).substring(2, 15));
            ...
            setTimer(prev => {
                ...
                if (newTime >= 60) {
                    ...
                    setTimeout(() => {
                        ...
                        console.log('Timer timeout - finalText:', finalText);
                        if (finalText) {
                            const cleanText = finalText.replace(/(\\b\\w+\\b(?:\\s+\\b\\w+\\b)*?)\\s+\\1+/g, '$1');
                            console.log('Timer timeout - cleanText:', cleanText);
                            sendMessage(cleanText);
                        } else {
                            console.log('Timer timeout - No final text to send');
                        }
                    }, 100);
    ...
    const sendMessage = async (textMessage = message) => {
        console.log('sendMessage called with:', textMessage);
    ...
    const stopRecording = () => {
        if (mediaRecorderRef.current && isRecordingRef.current && !isSubmitting) {
            ...
            setTimeout(() => {
                const finalText = accumulatedTextRef.current.trim();
                console.log('stopRecording - finalText:', finalText);
                if (finalText) {
                    const cleanText = finalText.replace(/(\\b\\w+\\b(?:\\s+\\b\\w+\\b)*?)\\s+\\1+/g, '$1');
                    console.log('stopRecording - cleanText:', cleanText);
                    sendMessage(cleanText);
                } else {
